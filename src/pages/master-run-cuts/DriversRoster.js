...
const DriversRoster = () => {
  ...
  const addDriver = async (event) => {
    ...
    if (!newDriver.name.trim() || !newDriver.division) return;
    setSaving(true);
    setError("");
    try {
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white">
        <table className="min-w-full divide-y divide-slate-200 text-sm">
          <thead className="bg-slate-50">
            <tr>
              {["Driver", "Pullout Address", "Division", "Status", "Actions"].map((heading) => (
                <th key={heading} className="px-3 py-2 text-left font-medium text-slate-500">{heading}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {loading && <tr><td colSpan={5} className="px-3 py-6 text-center text-slate-400">Loading drivers…</td></tr>}
            {!loading && sortedDrivers.length === 0 && <tr><td colSpan={5} className="px-3 py-6 text-center text-slate-400">No drivers in this roster yet.</td></tr>}
