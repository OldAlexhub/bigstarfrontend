...
const StandbyUtilization = () => {
  ...
  useEffect(() => {
    ...
    setError("");
    apiGet(
      `/api/run-cut-days?division=${selectedDivision._id}&from=${date}&to=${date}&includeStandby=1&sharedStandby=1`
    )
      .then((data) => {
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Calculated from the selected division's shared branch pool.</p>
        </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white">
        <table className="min-w-full divide-y divide-slate-200 text-sm">
          <thead className="bg-slate-50">
            <tr>
              {["Standby Route", "Home Branch", "Operator", "Status", "Utilized", "Covering Route"].map((heading) => (
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {!loading && standbyDays.map((day) => (
              <tr key={day._id}>
                ...
                <td className="px-4 py-2 text-slate-600">{day.division?.name || selectedDivision.name}</td>
