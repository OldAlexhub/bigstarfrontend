...
const SafetyLayout = () => {
  ...
  <div>
    ...
    <div className="mb-7 flex gap-6 border-b border-slate-200">
      {canAccessPage(user, "safety.accidents") && <NavLink to="/safety" end className={tabClasses}>
        Accidents
      </NavLink>}
      {canAccessPage(user, "safety.scores") && <NavLink to="/safety/scores" className={tabClasses}>
        Safety Scores
      </NavLink>}
      {canAccessPage(user, "safety.analytics") && <NavLink to="/safety/analytics" className={tabClasses}>
        Analytics
      </NavLink>}
    </div>
    <Outlet />
